...
const appfilesFolderPath = cmdArgs.get("appfiles") || "./appfiles/";
const outputFolderPath = cmdArgs.get("output") || "./build/";
const buildProject = async () => {
    ...
    for (const file of output) {
        await fs.writeFile(path.resolve(absoluteAppfilesFolderPath, file.fileName), file.type === "chunk" ? file.code : file.source, { encoding: "utf-8" });
        if (file.type === "chunk") {
...
if (cmdArgs.get("watch")) {
    ...
    (async () => {
        ...
        for await (const { filename, eventType } of fs.watch(path.resolve(process.cwd(), "./src/"), { recursive: true })) {
            if (performance.now() - lastChangeTime < 500)
                continue;
